(function () {
  ...
  let jobPage = 1, jobSearch = '', jobViewMode = 'kanban';
  let _appCfg = {};
  ...
  function tierBadge(tier) {
    const m={unitario:['badge-default','Unitario'],menudeo:['badge-regular','Menudeo'],mayoreo:['badge-frecuente','Mayoreo']};
    const [c,l]=m[tier]||['badge-default',tier]; return `<span class="badge ${c}">${l}</span>`;
  }
  function spoolCard(f, size=60) {
    const hex = f.color_hex || colorHex(f.color||'');
    if (typeof makeSpool === 'function' && f.material) {
      return makeSpool({marca:f.marca||'',material:f.material||'',color:f.color||'',color_hex:f.color_hex||'',acabado:f.acabado||''},size);
    }
    return `<div style="width:${size}px;height:${size}px;border-radius:50%;background:${hex};border:3px solid var(--border);display:flex;align-items:center;justify-content:center;font-size:${Math.round(size*0.2)}px;color:#fff;font-weight:700">${(f.material||'').substring(0,3)}</div>`;
  }
    ...
    el.innerHTML = `
      <div class="page-header">
        <div>
          ...
          <div class="page-subtitle">Órdenes de producción</div>
        </div>
      </div>
      <div class="fil-toolbar-top" style="margin-bottom:12px">
        ...
        <input class="search-input form-control" style="flex:1;max-width:280px" placeholder="Buscar trabajo..." oninput="jobSearch2(this.value)" autocomplete="off">
  ...
  async function refreshJobs() {
    ...
    jobPage=1; renderJobs();
  }

  window.jobSetView = function(mode) {
    jobViewMode=mode;
    ['list','kanban'].forEach(m=>{
      document.getElementById(`job-view-${m}`)?.classList.toggle('btn-primary',mode===m);
      document.getElementById(`job-view-${m}`)?.classList.toggle('btn-secondary',mode!==m);
    });
    renderJobs();
  };
  ...
  function renderKanban(filtered) {
    ...
    const cols = COLS.map(st=>{
      ...
      const cards = jobs.length===0
        ? `<div style="text-align:center;color:var(--text-muted);font-size:12px;padding:20px 8px">Sin trabajos</div>`
        : jobs.map(j=>{
            ...
            return `<div onclick="jobView(${j.id})" style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:8px;cursor:pointer;transition:border-color 0.15s"
              onmouseover="this.style.borderColor='${st.color}'" onmouseout="this.style.borderColor='var(--border)'">
              <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:6px;margin-bottom:4px">
                <div style="font-size:12px;font-weight:700;color:var(--text);line-height:1.3;flex:1">${j.nombre_proyecto||'Sin nombre'}</div>
                ${printerName?`<div style="font-size:10px;background:var(--surface);border:1px solid var(--border);border-radius:20px;padding:2px 7px;white-space:nowrap;color:var(--text-muted)">🖨️ ${printerName}</div>`:''}
              </div>
              ...
              ${pct!==null?`<div>
                ...
                <div style="height:5px;background:var(--border);border-radius:3px"><div style="height:100%;background:${st.color};border-radius:3px;width:${pct}%"></div></div>
              </div>`:''}
      ...
      return `<div style="flex:1;min-width:240px">
        <div style="background:${st.color}22;border:1px solid ${st.color}44;border-radius:12px;padding:10px 14px;margin-bottom:10px;display:flex;align-items:center;gap:6px">
          ...
          <span style="margin-left:auto;background:${st.color};color:#fff;font-size:11px;font-weight:700;padding:2px 8px;border-radius:99px">${jobs.length}</span>
        </div>
        ${cards}
      </div>`;
    }).join('');
    ...
    ${items.map(j=>{
      ...
      return `<div style="background:var(--card);border:1px solid var(--border);border-radius:14px;padding:16px;display:flex;flex-direction:column;gap:8px;cursor:pointer;transition:all 0.2s" onclick="jobView(${j.id})" onmouseover="this.style.borderColor='var(--accent)'" onmouseout="this.style.borderColor='var(--border)'">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
          <div style="font-weight:700;font-size:14px;flex:1">${j.nombre_proyecto||'Sin nombre'}${fallo}</div>
          ${stageBadge(j.estado)}
        </div>
        <div style="font-size:12px;color:var(--text-muted);display:flex;justify-content:space-between">
          <span>👥 ${j.cliente_nombre||'—'}</span><span>📅 ${j.fecha?j.fecha.substring(0,10):'-'}</span>
        </div>
        <div style="font-size:16px;font-weight:800;color:var(--accent-light)">${fmtMoney(j.precio_final)}</div>
        ${pct!==null?`<div><div style="display:flex;justify-content:space-between;font-size:10px;color:var(--text-muted);margin-bottom:3px"><span>Camas ${done}/${total}</span><span>${pct}%</span></div>
          <div style="height:5px;background:var(--border);border-radius:3px"><div style="height:100%;background:var(--accent);border-radius:3px;width:${pct}%"></div></div></div>`:''}
  ...
  function renderPaginationInline(cid,cur,total,fn) {
    const c=document.getElementById(cid); if(!c||total<=1){if(c)c.innerHTML='';return;}
    c.innerHTML='';
    const mk=(l,p,d,a)=>{const b=document.createElement('button');b.className='page-btn'+(a?' active':'');b.textContent=l;b.disabled=d;if(!d)b.addEventListener('click',()=>fn(p));c.appendChild(b);};
    mk('‹',cur-1,cur===1,false);
    for(let i=1;i<=total;i++){
      if(total>7&&i>2&&i<total-1&&Math.abs(i-cur)>1){if(i===3||i===total-2){const s=document.createElement('span');s.className='page-info';s.textContent='…';c.appendChild(s);}continue;}
      mk(String(i),i,false,i===cur);
    }
    mk('›',cur+1,cur===total,false);
  }
    ...
    try {
            ...
            <div style="display:flex;flex-wrap:wrap;gap:14px">
              ${fils.map(f=>{
                ...
                return `<div style="display:flex;flex-direction:column;align-items:center;gap:5px;min-width:72px">
                  ${spoolCard(f,62)}
                  <div style="font-size:10px;font-weight:700;text-align:center;max-width:80px;line-height:1.3">${f.nombre||(f.marca?f.marca+' ':'')+f.material||'—'}</div>
                  ...
                  ${warn?`<div style="font-size:9px;background:#ef444422;color:#ef4444;border-radius:4px;padding:1px 5px">⚠️ Stock: ${fmtNum(stock,0)}g</div>`
                       :(stock!==null?`<div style="font-size:9px;color:var(--text-muted)">Stock: ${fmtNum(stock,0)}g</div>`:'')}
                </div>`;
              }).join('')}
            </div>
          </div>`;
      ...
      if (camas.length>0) {
        ...
        html+=`<div style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:14px;margin-bottom:12px">
          ...
          ${camas.map(c=>`<div style="display:flex;align-items:center;gap:10px;padding:8px 4px;border-bottom:1px solid var(--border);${idx>=2?'cursor:pointer':''}"
            ${idx>=2?`onclick="jobToggleCama(${id},${c.id})"`:''}>
            <span style="font-size:20px">${c.completada?'✅':'⬜'}</span>
            <span style="flex:1;font-size:13px;${c.completada?'text-decoration:line-through;opacity:0.5':''}">Cama ${c.numero}${c.descripcion?' — '+c.descripcion:''}</span>
            ${c.completada_at?`<span style="font-size:10px;color:var(--text-muted)">${c.completada_at}</span>`:''}
          </div>`).join('')}
          ${idx>=2?'<div style="font-size:10px;color:var(--text-muted);margin-top:6px;text-align:center">Toca una cama para marcarla completa</div>':''}
        </div>`;
      }
  ...
  window.jobGoToLevantamiento = async function(id) {
    try { await api('PUT',`/api/jobs/${id}`,{estado:'Levantamiento'}); await jobOpenForm(id); }
    catch(e) { showToast('Error: '+e.message,'error'); }
  };
  window.jobJustAdvance = async function(id,stage) {
    try { await api('PUT',`/api/jobs/${id}`,{estado:stage}); showToast(`Avanzado a ${stage}`); await refreshJobs(); }
    catch(e) { showToast('Error: '+e.message,'error'); }
  };
  window.jobToggleCama = async function(jobId,camaId) {
    try { await api('PATCH',`/api/jobs/${jobId}/camas/${camaId}`,{}); closeModal(); await refreshJobs(); await jobView(jobId); }
    catch(e) { showToast('Error: '+e.message,'error'); }
  };
  ...
  function renderSolicitudFields(j) {
    const clientOpts=allClients.map(c=>`<option value="${c.id}" ${j.cliente_id==c.id?'selected':''}>${c.nombre}</option>`).join('');
    return `<div style="font-weight:700;color:var(--accent-light);margin-bottom:12px">📥 Solicitud</div>
      <div class="form-grid" style="margin-bottom:16px">
        ...
        <div class="form-group"><label>Cliente</label><select class="form-control" name="cliente_id"><option value="">— Sin cliente —</option>${clientOpts}</select></div>
  ...
  window.jlevFilChange = function(i) {
    ...
    const spoolEl=document.getElementById(`jlev-spool-${i}`);
    if (spoolEl) spoolEl.innerHTML=fil?spoolCard({...fil},48):`<div style="width:48px;height:48px;border-radius:50%;background:var(--border);border:2px solid var(--border)"></div>`;
    jlevRecalc();
    const gEl=document.getElementById(`jlev-fil-g-${i}`);
    const stock=fil?fil.peso_actual_g:null;
    const g=parseFloat(gEl?.value)||0;
    const stockEl=document.getElementById(`jlev-stock-${i}`);
    if (stockEl) { const warn=stock!==null&&g>0&&stock<g; stockEl.style.color=warn?'#ef4444':'var(--text-muted)'; stockEl.textContent=stock!==null?(warn?`⚠️ Stock: ${fmtNum(stock,0)}g`:`✓ ${fmtNum(stock,0)}g`):''; }
  };
  window.jlevAddFil = function() {
    const list=document.getElementById('jlev-fils'); if(!list)return;
    const wrap=document.createElement('div'); wrap.innerHTML=jlevFilRowHtml(_jlevFilCount,null,''); list.appendChild(wrap.firstChild); _jlevFilCount++;
  };
  ...
  window.jlevRecalc = function() {
    ...
    for(let i=0;i<_jlevFilCount;i++){
      ...
      const stockEl=document.getElementById(`jlev-stock-${i}`);
      if(stockEl&&fil){const warn=fil.peso_actual_g<g&&g>0;stockEl.style.color=warn?'#ef4444':'var(--text-muted)';stockEl.textContent=warn?`⚠️ Stock: ${fmtNum(fil.peso_actual_g,0)}g`:`✓ ${fmtNum(fil.peso_actual_g,0)}g`;}
    }
    const kwh=pr?((pr.consumo_promedio_watts||0)/1000)*hImp:0;
    const cLuz=kwh*(parseFloat(_appCfg.costo_kwh)||0.18);
    const cMaq=pr?hImp*(pr.costo_por_hora||0):0;
      ...
      <div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px">
          <div class="form-group" style="margin:0">
            <label style="font-size:11px">Impresión (h)</label>
            <div style="display:flex;gap:4px">
              <input class="form-control" id="jlev-h" type="number" min="0" value="${hImp}" placeholder="0" style="width:100%" oninput="jlevRecalc()">
            </div>
          </div>
          <div class="form-group" style="margin:0">
            <label style="font-size:11px">Impresión (min)</label>
            <input class="form-control" id="jlev-m" type="number" min="0" max="59" value="${mImp}" placeholder="0" oninput="jlevRecalc()">
          </div>
          <div class="form-group" style="margin:0">
            <label style="font-size:11px">Mano de obra (h)</label>
            <input class="form-control" id="jlev-mo-h" type="number" min="0" value="${hMO}" placeholder="0" oninput="jlevRecalc()">
          </div>
          <div class="form-group" style="margin:0">
            <label style="font-size:11px">Mano de obra (min)</label>
            <input class="form-control" id="jlev-mo-m" type="number" min="0" max="59" value="${mMO}" placeholder="0" oninput="jlevRecalc()">
          </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px">
          <div class="form-group" style="margin:0">
            ...
            <input class="form-control" id="jlev-embalaje" type="number" step="any" value="${emb}" placeholder="0.00" oninput="jlevRecalc()">
          </div>
          <div class="form-group" style="margin:0">
          ...
          <div class="form-group" style="margin:0">
            <label style="font-size:11px">💵 Precio final ($)</label>
            <input class="form-control" id="jlev-precio-final" name="precio_final" type="number" step="any"
              value="${j.precio_final||''}" placeholder="0.00" autocomplete="off" oninput="this.dataset.manual='1'">
          </div>
          <div class="form-group" style="margin:0">
            <label style="font-size:11px">🛏️ Número de camas</label>
            <input class="form-control" id="jlev-n-camas" type="number" min="1" value="${nCamas}">
          </div>
        </div>

        <!-- Desglose costo -->
        <div style="background:var(--surface);border-radius:10px;padding:12px;font-size:12px">
  ...
  function renderProduccionFields(j) {
    ...
    return `<div style="font-weight:700;color:var(--accent-light);margin:4px 0 12px">🖨️ Producción</div>
      ...
      ${camas.length?`
        <div style="font-size:11px;color:var(--text-muted);font-weight:600;text-transform:uppercase;margin-bottom:8px">🛏️ CAMAS (${done}/${camas.length})</div>
      ...
      `:`<div style="color:var(--text-muted);font-size:12px;padding:8px 0">Sin camas definidas — defínelas en el Levantamiento</div>`}`;
  }
  ...
  window.jobOpenForm = async function(id, calcPrefill) {
    ...
    if (!id) {
      ...
      buttonsHtml=`<button type="button" class="btn btn-secondary" onclick="cancelModal()">Cancelar</button>
      ...
      if (stage.key==='Solicitud') {
        ...
        buttonsHtml=`<button type="button" class="btn btn-secondary" onclick="cancelModal()">Cancelar</button>
          <button type="button" class="btn btn-primary" onclick="jobSave(${id})">💾 Guardar</button>
          <button type="button" class="btn btn-success" onclick="jobSave(${id},'Cierre')">✅ Cerrar trabajo</button>`;
      } else {
  ...
  window.jobAddProduct = function() {
    const l=document.getElementById('products-list'); if(!l)return;
    const r=document.createElement('div'); r.className='extra-item';
    r.innerHTML=`<input class="form-control" name="prod_desc[]" placeholder="Descripción" style="flex:2" autocomplete="off"><input class="form-control" name="prod_qty[]" type="number" min="1" value="1" style="width:70px" autocomplete="off"><button type="button" class="btn btn-danger btn-sm" onclick="this.parentElement.remove()">✕</button>`;
    l.appendChild(r);
  };
  window.jobAddExtra = function() {
    const l=document.getElementById('extras-list'); if(!l)return;
    const r=document.createElement('div'); r.className='extra-item';
  ...
  window.jobSave = async function(id, advanceTo) {
    ...
    // Validate: impresora requerida en levantamiento
    const prSel=document.getElementById('jlev-printer-sel');
    if (prSel&&!prSel.value) {
      showToast('Selecciona una impresora antes de guardar','error'); prSel.focus(); return;
    }
    ...
    if (prSel) {
      ...
      for(let i=0;i<_jlevFilCount;i++){
        ...
        fils.push({fil_id:parseInt(sel.value),filamento_id:parseInt(sel.value),gramos:parseFloat(gEl?.value)||0,gramos_pieza:parseFloat(gEl?.value)||0,
          ...
          color:fil?.color||'',color_hex:fil?.color_hex||'',material:fil?.material||'',acabado:fil?.acabado||'',marca:fil?.marca||'',nombre_comercial:fil?.nombre_comercial||''});
      }
