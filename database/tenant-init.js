...
async function initTenantDb(slug, adminUsername, adminPassword, adminDisplayName) {
  const tenantsDir = path.join(__dirname, 'tenants');
  fs.mkdirSync(tenantsDir, { recursive: true });

  const dbPath = path.join(tenantsDir, `${slug}.db`);
  const db = makeDbHelpers(new sqlite3.Database(dbPath));
  ...
  const jobLevColumns = ['levantamiento_datos TEXT', 'cotizacion_id INTEGER', 'archivado INTEGER DEFAULT 0', 'archivado_at TEXT', 'canal_venta TEXT', 'orden_id TEXT', 'cobrado INTEGER DEFAULT 1'];
  ...
  if (userCount.cnt === 0) {
    ...
    await db.runAsync(
      ...
      [adminUsername.toLowerCase().trim(), adminDisplayName || adminUsername, hash, 'admin']
    );
  }
...
module.exports = { initTenantDb, makeDbHelpers };
