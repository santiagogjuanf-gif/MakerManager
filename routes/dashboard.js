...
router.get('/', async (req, res) => {
  try {
    ...
    // Jobs by stage
    const jobsByStage = await db.allAsync(
      'SELECT estado, COUNT(*) as cnt FROM print_jobs GROUP BY estado'
    );

    // Monthly revenue: last 6 complete months + current
    ...
    // Top filaments used this month by grams
    const topFilaments = await db.allAsync(`
