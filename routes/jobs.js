...
async function getConfig(dbInstance) {
  const rows = await dbInstance.allAsync('SELECT key, value FROM config');
  const cfg = {};
  rows.forEach(r => cfg[r.key] = r.value);
  return cfg;
}

async function calcJobCost(jobId, dbInstance) {
  if (!dbInstance) dbInstance = require('../database/db');
  const job = await dbInstance.getAsync('SELECT * FROM print_jobs WHERE id=?', [jobId]);
  if (!job) return null;
  const printer = job.impresora_id ? await dbInstance.getAsync('SELECT * FROM printers WHERE id=?', [job.impresora_id]) : null;
  const jobFilaments = await dbInstance.allAsync(`
  ...
  const extras = await dbInstance.allAsync('SELECT * FROM job_extras WHERE print_job_id=?', [jobId]);
  const products = await dbInstance.allAsync('SELECT * FROM job_products WHERE print_job_id=?', [jobId]);
  const cfg = await getConfig(dbInstance);
...
router.get('/', async (req, res) => {
  try {
    ...
    `);
    for (const j of jobs) {
      j.filaments = await req.db.allAsync(
      ...
      j.products = await req.db.allAsync('SELECT * FROM job_products WHERE print_job_id=?', [j.id]);
    }
    res.json(jobs);
...
router.get('/:id', async (req, res) => {
  try {
    ...
    job.filaments = await req.db.allAsync(
    ...
    job.products = await req.db.allAsync('SELECT * FROM job_products WHERE print_job_id=?', [req.params.id]);
    job.extras = await req.db.allAsync('SELECT * FROM job_extras WHERE print_job_id=?', [req.params.id]);
    job.camas = await req.db.allAsync('SELECT * FROM job_camas WHERE print_job_id=? ORDER BY numero ASC', [req.params.id]);
    res.json(job);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
async function saveJobData(jobId, d, dbInstance) {
  // Save filaments
  await dbInstance.runAsync('DELETE FROM job_filaments WHERE print_job_id=?', [jobId]);
  ...
  // Save products
  await dbInstance.runAsync('DELETE FROM job_products WHERE print_job_id=?', [jobId]);
  ...
  // Save extras
  await dbInstance.runAsync('DELETE FROM job_extras WHERE print_job_id=?', [jobId]);
  ...
  if (d.camas && Array.isArray(d.camas)) {
    await dbInstance.runAsync('DELETE FROM job_camas WHERE print_job_id=?', [jobId]);
...
async function deductInventory(d, sign, dbInstance) {
  if (!dbInstance) dbInstance = require('../database/db');
  ...
  if (d.filaments.length > 0 && d.filaments[0].filamento_id) {
    ...
    if (extra > 0) {
      await dbInstance.runAsync('UPDATE filaments SET peso_actual_g = MAX(0, peso_actual_g + ?) WHERE id=?',
...
router.patch('/:id/camas/:camaId', async (req, res) => {
  try {
    const cama = await req.db.getAsync('SELECT * FROM job_camas WHERE id=? AND print_job_id=?', [req.params.camaId, req.params.id]);
...
router.post('/', async (req, res) => {
  try {
    ...
    if (d.cliente_id) {
      await req.db.runAsync('UPDATE clients SET total_pedidos = total_pedidos + 1 WHERE id=?', [d.cliente_id]);
      await updateClassification(d.cliente_id, req.db);
    }
...
router.put('/:id', async (req, res) => {
  try {
    const d = req.body;
    const old = await req.db.getAsync('SELECT * FROM print_jobs WHERE id=?', [req.params.id]);
    if (!old) return res.status(404).json({ error: 'Not found' });
    const oldFilaments = await req.db.allAsync('SELECT * FROM job_filaments WHERE print_job_id=?', [req.params.id]);

    const merged = { ...old, ...d };

    await deductInventory({ filaments: oldFilaments.map(f=>({filamento_id:f.filamento_id,gramos_pieza:f.gramos_pieza})), gramos_purga: old.gramos_purga, gramos_perdidos: old.gramos_perdidos, fallo: old.fallo }, +1, req.db);
    if (old.impresora_id && old.tiempo_impresion_min > 0) {
      await req.db.runAsync('UPDATE printers SET horas_acumuladas = MAX(0, horas_acumuladas - ?) WHERE id=?',
    ...
    await deductInventory({ ...merged, filaments: d.filaments !== undefined ? d.filaments : oldFilaments.map(f=>({filamento_id:f.filamento_id,gramos_pieza:f.gramos_pieza})) }, -1, req.db);
    if (merged.impresora_id && merged.tiempo_impresion_min > 0) {
      await req.db.runAsync('UPDATE printers SET horas_acumuladas = horas_acumuladas + ? WHERE id=?',
...
router.delete('/:id', async (req, res) => {
  try {
    const job = await req.db.getAsync('SELECT * FROM print_jobs WHERE id=?', [req.params.id]);
    if (job) {
      const filaments = await req.db.allAsync('SELECT * FROM job_filaments WHERE print_job_id=?', [req.params.id]);
      await deductInventory({ filaments: filaments.map(f=>({filamento_id:f.filamento_id,gramos_pieza:f.gramos_pieza})), gramos_purga: job.gramos_purga, gramos_perdidos: job.gramos_perdidos, fallo: job.fallo }, +1, req.db);
      if (job.impresora_id && job.tiempo_impresion_min > 0) {
        await req.db.runAsync('UPDATE printers SET horas_acumuladas = MAX(0, horas_acumuladas - ?) WHERE id=?',
          [job.tiempo_impresion_min/60, job.impresora_id]);
      }
      if (job.cliente_id) {
        await req.db.runAsync('UPDATE clients SET total_pedidos = MAX(0, total_pedidos - 1) WHERE id=?', [job.cliente_id]);
        await updateClassification(job.cliente_id, req.db);
      }
...
router.patch('/:id/cobrado', async (req, res) => {
  try {
    const job = await req.db.getAsync('SELECT cobrado FROM print_jobs WHERE id=?', [req.params.id]);
    if (!job) return res.status(404).json({ error: 'Not found' });
