...
router.get('/', async (req, res) => {
  try {
    const rows = await db.allAsync('SELECT id, nombre, descripcion, foto_path, precio_online, precio_local, created_at FROM productos ORDER BY nombre ASC');
    res.json(rows);
...
router.put('/:id', async (req, res) => {
  try {
    const { nombre, descripcion, datos, precio_online, precio_local } = req.body;
    const existing = await db.getAsync('SELECT id FROM productos WHERE id=?', [req.params.id]);
...
router.post('/:id/foto', upload.single('foto'), async (req, res) => {
  try {
    ...
    const existing = await db.getAsync('SELECT foto_path FROM productos WHERE id=?', [req.params.id]);
    ...
    await db.runAsync('UPDATE productos SET foto_path=? WHERE id=?', [foto_path, req.params.id]);
    res.json({ foto_path });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.delete('/:id', async (req, res) => {
  try {
    const row = await db.getAsync('SELECT foto_path FROM productos WHERE id=?', [req.params.id]);
    if (row?.foto_path) {
      const p = path.join(__dirname, '../public', row.foto_path);
      if (fs.existsSync(p)) fs.unlinkSync(p);
    }
    await db.runAsync('DELETE FROM productos WHERE id=?', [req.params.id]);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
